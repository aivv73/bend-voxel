...
int main() {
  ...
  frame.aim[0]=.05f; frame.aim[1]=2.35f; frame.aim[2]=.05f;
  frame.aim_kind=1;
  frame.face_count=2;
  frame.faces=faces;
  frame.hud="FRAME 1";
  ...
  const auto scene_vertices=cache.scene_vertices;
  frame.hud="FRAME 2";
  ...
  frame.aim_kind=0;
  expect_fresh_geometry(frame,cache,true);
  assert(cache.geometry.triangles==scene_vertices);
  frame.aim_kind=1;
  expect_fresh_geometry(frame,cache,true);
  assert(cache.scene_vertices==scene_vertices);
  faces[0].offset=.1f;
  expect_fresh_geometry(frame,cache,false);
  expect_fresh_geometry(frame,cache,true);
  frame.face_count=1;
