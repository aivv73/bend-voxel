...
class SampleTests(unittest.TestCase):
    def rows(self):
        return ('frame,4999000,4999000,4640,0\n'
                ...
                'stage,5000000,100,200,0,100,100,0,400,100\n'
                'edit,5000000,4294967200,101,1,100,200\n'
                'cut,4294967200,900,1,101\n'
                'frame,65300000,60300000,4622,1\n'
                'stage,65300000,0,0,0,0,60000000,0,300000,0\n')
    ...
    def test_missing_or_corrupt_stage_fails(self):
        for bad in (self.rows().replace('stage,5000000,100,200,0,100,100,0,400,100\n',''),
                    self.rows().replace(',400,100\n', ',400,101\n'),
                    self.rows().replace('101,1,100,200', '101,1,101,200')):
            self.assertFalse(parse_run(io.StringIO(bad))['instrumentation_pass'])

    def test_multiple_edits_per_frame(self):
        rows = self.rows().replace('edit,5000000,4294967200,101,1,100,200',
            'edit,5000000,4294967200,101,1,40,80\nedit,5000000,42,102,1,60,120\ncut,42,800,1,102')
