...
Run the separate throughput suite after installing the demo's build dependencies:
...
The world stays within the current 40 × 24 × 20 lattice. The dense and full cases increase traversal and edit work. The comb exposes many surface rectangles. Render copies process and draw the same faces at the same positions, deliberately increasing geometry generation, upload, and overdraw; they do **not** add simulated cells or bodies. `face_visits` counts input rectangles times copies, before view culling. This suite characterizes the current bounded implementation, rather than establishing an unbounded world scale limit.

The stress runner requests Vulkan immediate presentation, falling back to mailbox. It fails if neither is available, records the selected mode, and does not silently publish FIFO-paced throughput. It does not change the presentation mode of `make run` or `make benchmark`.
...
Use `--cases`, `--warmup`, `--frames`, `--edit-every`, and `--output` to select a smaller or larger run. `--edit-every 0` measures steady rendering without carving. The existing `make benchmark` remains the fixed acceptance replay with its own performance gates.
