...
STAGES = ('carving', 'connectivity_meshing', 'update_other', 'scene_preparation',
...
def parse_run(lines, exit_code=0):
    ...
    for row in csv.reader(lines):
        ...
        try:
            ...
            expected = {'frame': 4, 'stage': 9, 'edit': 6, 'cut': 4, 'attempt': 4}[tag]
    ...
    for t, (duration, _, _) in frames.items():
        ...
        if t in stages and [sum(e[i] for e in own) for i in (4, 5)] != stages[t][:2]:
    ...
    return {'exit_code': exit_code, 'complete_window': complete, 'expected_accepted_cuts': 33,
            ...
            'active_edits': {name: stats([e[i]/1000 for e in edits if e[0] >= 5_000_000 and e[3] == 1]) for name, i in [('carving', 4), ('connectivity_meshing', 5)]},
