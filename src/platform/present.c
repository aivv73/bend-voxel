// Adapted from Bend 2.0.16 Window.frame: focus release and presentation synchronization.
// Window
// ======
...
static void window_pump(BendWin* win) {
  ...
  XSelectInput(win->dpy, win->win, KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask | FocusChangeMask | LeaveWindowMask);
...
static void window_show(Env e, BendWin* win, Term image) {
  u32 w = win->img->width;
  u32 h = win->img->height;
  // Bend3D Frame.show always returns a 2048-square image tree.
  u32 k = 11;
...
Term present_run(Env e, Term* f, IoWork* w) {
  ...
  u64 length = 0;
  char* text = io_cstr(e, f[2], &length);
  ...
  for (int line = 0; line < 6 && start; ++line) {
    ...
    XSetForeground(win->dpy, gc, colors[line]);
    XDrawString(win->dpy, win->win, gc, 12, ys[line], start, end ? (int)(end - start) : (int)strlen(start));
    start = end ? end + 1 : NULL;
  }
  XSetForeground(win->dpy, gc, 0x4bc1a4);
  XDrawString(win->dpy, win->win, gc, 577, 20, "RESET", 5);
  free(text);
  XSync(win->dpy, False);
#endif
...
static void __attribute__((constructor)) present_use(void) {
  io_eff(CID_PRESENT, present_run, 0);
}
